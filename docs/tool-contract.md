...
The conventions every tool in both servers follows. They are shaped by the caller: a language model
with a finite context window and no cheap way to ask a follow-up.
...
`Run` (in `TeamsTools.cs` / `AdoTools.cs`):
...
The log reference is literally `(details: grep "req=7" in C:\…\teams-mcp.log)`, so a pasted error
leads straight to the lines that explain it.
...
A call that cannot bind its arguments never enters `Run`. The SDK's binder raises that failure
above it, past the call-tool filter, and the SDK's composed handler catches it one frame higher and
replaces it with `An error occurred invoking '<tool>'.` No `req=N`, no log line naming the tool, no
hint that the parameter wanted was `release_id` and not `releaseId`. (The SDK's literal carries the
detail after a colon; what ships has a period and nothing after it.)

`ToolErrors.Guard` (an `AddCallToolFilter`, wired beside `ToolResults.Trim`) closes that, in the
order the cases fire:

| Case | What `Guard` does |
| --- | --- |
| The supplied names cannot bind to the tool's `inputSchema` | Refuses **before dispatch**, naming the tool, the unknown argument, the missing required one, the full parameter list and what was supplied. An unknown name that is a real parameter in the wrong convention is called out as such, the one case that reports both faults at once |
...
`Run`'s `req=N` and log reference, and the second is not a failure. `McpProtocolException` derives
from `McpException`, so an unknown tool or method name stays a JSON-RPC error instead of a tool
result claiming the call was dispatched. It is the one failure class with no `req=N`, and
`ServerInstructions` says so.

`Guard` allocates from the same static counter `Run` does, so a failure caught either side of `Run`
reads the same in the log. Its validation is the check `Call.Coerce` already makes for the command
line. Both servers carry their own copy, per the deliberate-duplication rule.

## Output is optimized for a context window

`Program.cs` configures the serializer once with
`DefaultIgnoreCondition = JsonIgnoreCondition.WhenWritingNull`, not attributes on the DTOs. So the
convention is: make a field nullable and set it to `null` when it is uninteresting, and it
...
The server instructions tell the model how to read that silence: an omitted field means "nothing to
say", not "unknown".
...
DevOps server's release definitions hold `whsec_` and `sk_` values. Leak one into a transcript and
it cannot be un-said: the transcript outlives the call.

The rule is about the shape of a response, not about particular endpoints, so it holds where a type
does not:
...
- `ado_api_request` walks the parsed response and replaces the `value` of any object carrying
  `isSecret: true` with `[redacted]` (`ApiRequest.Mask`): the same rule on a body this server has no
  type for.
...
- One thing is withheld that the service does *not* mark: a deployment agent's capabilities, which
  ...
  That withholds a whole bag rather than guessing at values, so it is not the heuristic ruled out
  below.
...
Records that never ran are neither listed nor counted.

Counting happens during mapping, before any cap, so the counts cover everything examined and not
only what fit.
...
New id parameters do the same; in the Azure DevOps server that means calling `Resolve`, not writing
another.

## Bodies become plain text

`TeamsTools.HtmlToText` and `Text.FromHtml` / `Text.FromMarkdown` are `[GeneratedRegex]` pipelines
that keep what an agent acts on and drop the rest:
...
Truncation happens **after** conversion, at `body_limit`, flagged with `truncated: true`. It cuts
back one character when the limit would split a surrogate pair, since half an emoji is an invalid
character.
...
**repeats the gate's environment variable in its `[Description]`**. It needs both: a client gates
its confirmation prompt on the annotation, the model reads the description, so a refusal reads as
...
unset one is omitted instead of sent as `false`, the same omit-what-is-uninteresting rule the DTOs
follow. `OpenWorld` stays unset in both servers: the spec's default is already `true`, right for a
remote organization or tenant.
...
| `deploy_release`, `approve_release` | `Destructive = true, Idempotent = false`. Deploying replaces what is running in that environment, and approving a pre-deploy gate is what lets it happen |

A tool that sets neither `ReadOnly` nor a mutation hint fails `ToolListingTests`.

## Structured content, minus the duplicate text copy

`UseStructuredContent = true` on every tool is what makes the SDK generate an `outputSchema`. A
model learns the shape of a result before spending a call to find out, so a chain like
`get_pull_request` → `add_pull_request_comment` can be planned.

The flag also makes the SDK send the payload **twice**: as escaped JSON in a text block and as
native JSON in `structuredContent`. Measured, they are byte-identical, the text copy larger because
it escapes every quote. `ToolResults.Trim`, wired as an `AddCallToolFilter`, drops the text copy,
leaving the schema as the whole net cost.
...
`ToolListing.Prepare`, wired as an `AddListToolsFilter` so the assembly scan stays the source of the
...
`Prepare` also sorts the listing, which the spec asks for so a client can cache it and a model's
...
better part of an hour. Both servers enable the Tasks extension (`.WithTasks`, SEP-2663) with an
`InMemoryMcpTaskStore`, which is correct for stdio: the store dies with the client and there is
nothing worth persisting.

Only the waiters are task-capable. `ExecutionModeSelector` marks them `Optional` and every other
tool `Synchronous`: handing back a handle the caller has to chase is a worse answer than a
sub-second result.

`Optional` rather than `Required` lets a client that never negotiated the extension still work: it
blocks instead. So **each waiter bounds its own wait** and never relies on the client to give up.
Timeouts and poll intervals are clamped in code.
...
A waiter that runs out of time returns its result with `timedOut: true` instead of throwing. "It has
not finished" and "it failed" are different answers. `req=N` correlates throughout, because `Run`
sets it inside the tool body and it flows to every poll; only `tasks/get` arrives as a separate
request with its own id.

## Parameter naming

Tool parameter names are `snake_case` (`include_replies`, `body_limit`, `target_branch`) because
that is what reaches the model. C# locals and DTO members stay PascalCase/camelCase. The serializer
handles the DTO casing; nothing handles the parameter casing, so it is spelled that way in the
signature.

## Server instructions carry what a tool description cannot

`ServerInstructions` in each `Program.cs` is sized like a system prompt: how the server fails, what
