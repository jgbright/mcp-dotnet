...
/// <summary>
/// A broken log must not break the server, so the failure paths matter more than the happy one.
/// </summary>
public class FileLineSinkTests : IDisposable
{
    private readonly string _dir = Path.Combine(
        Path.GetTempPath(), "ado-mcp-tests", Guid.NewGuid().ToString("N"));

    private string LogPath(string name = "ado-mcp.log") => Path.Combine(_dir, name);
    ...
    {
        using var sink = new FileLineSink(Path.Combine(_dir, "nested", "deep", "ado-mcp.log"));

        sink.Write("first");

        Assert.Equal(["first"], ReadAllLines(Path.Combine(_dir, "nested", "deep", "ado-mcp.log")));
    }
