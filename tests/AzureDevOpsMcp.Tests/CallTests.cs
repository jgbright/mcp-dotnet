...
public class CallTests
{
    ...
    public void Pairs_coerce_to_what_the_schema_declares()
    {
        var arguments = Call.ParseArguments(
            Schema, ["project=Core", "limit=5", "ratio=0.5", "active=true", "top=3"], NoStdin);

        Assert.Equal("Core", Assert.IsType<string>(arguments["project"]));
        Assert.Equal(5L, Assert.IsType<long>(arguments["limit"]));
        Assert.Equal(0.5, Assert.IsType<double>(arguments["ratio"]));
        Assert.True(Assert.IsType<bool>(arguments["active"]));
        Assert.Equal(3L, Assert.IsType<long>(arguments["top"]));
    }

    [Fact]
    public void An_array_argument_is_taken_as_json()
    {
        var arguments = Call.ParseArguments(Schema, ["""tags=["a","b"]"""], NoStdin);

        Assert.Equal(JsonValueKind.Array, Assert.IsType<JsonElement>(arguments["tags"]).ValueKind);
    }

    [Fact]
    public void A_value_may_contain_the_separator()
    {
        var arguments = Call.ParseArguments(Schema, ["project=a=b"], NoStdin);

        Assert.Equal("a=b", arguments["project"]);
    }
    ...
    public void An_unknown_key_fails_listing_what_the_tool_takes()
    {
        var e = Assert.Throws<FormatException>(
            () => Call.ParseArguments(Schema, ["projcet=Core"], NoStdin));

        Assert.Contains("projcet", e.Message);
        Assert.Contains("project", e.Message);
    ...
    {
        var e = Assert.Throws<FormatException>(
            () => Call.ParseArguments(Schema, ["project", "Core"], NoStdin));
    ...
    {
        var arguments = Call.ParseArguments(
            Schema, ["""{"project":"Core","limit":5}"""], NoStdin);

        Assert.Equal("Core", Assert.IsType<JsonElement>(arguments["project"]).GetString());
    ...
    public void Broken_json_is_a_terminal_message_not_a_stack_trace()
    {
        Assert.Throws<FormatException>(
            () => Call.ParseArguments(Schema, ["""{"project": """], NoStdin));
        Assert.Throws<FormatException>(
    ...
    public void A_structured_result_is_the_only_thing_on_stdout()
    {
        ...
        {
            Content = [],
            StructuredContent = JsonSerializer.SerializeToElement(new { projects = new[] { "Core" } }),
        });

        Assert.Equal(0, code);
        Assert.Empty(stderr);
        var parsed = JsonSerializer.Deserialize<JsonElement>(stdout);
        Assert.Equal("Core", parsed.GetProperty("projects")[0].GetString());
    }
    ...
    public void An_error_lands_on_stderr_and_exits_nonzero()
    {
        ...
        {
            IsError = true,
            Content = [new TextContentBlock { Text = "no such project. req=3" }],
        });
    ...
    public void An_exact_name_wins_even_when_it_is_also_a_substring_of_another()
        => Assert.Equal("get_work_item", Call.ResolveTool(["get_work_item", "get_work_items"], "get_work_item"));

    [Fact]
    public void A_unique_substring_resolves_like_the_server_s_own_resolvers()
        => Assert.Equal("list_repos", Call.ResolveTool(Names, "repos"));

    [Fact]
    public void Case_never_matters()
        => Assert.Equal("list_projects", Call.ResolveTool(Names, "LIST_PROJECTS"));

    [Fact]
    public void An_ambiguous_name_fails_listing_only_the_candidates()
    {
        var e = Assert.Throws<FormatException>(() => Call.ResolveTool(Names, "work_item"));

        Assert.Contains("get_work_item", e.Message);
        Assert.Contains("search_work_items", e.Message);
        Assert.DoesNotContain("list_repos", e.Message);
    }
    ...
    public void No_match_fails_listing_every_tool()
    {
        ...
        Assert.Contains("list_projects", e.Message);
