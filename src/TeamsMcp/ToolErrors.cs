...
internal static class ToolErrors
{
    ...
    private static CallToolResult Fail(
        ...
        Exception? thrown)
    {
        var req = TeamsTools.NextRequest();
        ...
        try
        {
            log?.Line(thrown is null ? LogLevel.Warning : LogLevel.Error, Ev.ToolFail,
                $"{tool ?? "?"} rejected" +
                TeamsMcpLog.Arg("supplied", supplied.Count > 0 ? string.Join(",", supplied) : "<none>") +
                TeamsMcpLog.Arg("reason", headline),
                thrown);
        }
        finally
        {
        ...
        message.Append(supplied.Count > 0
            ...
            : " Supplied: nothing.");
        message.Append(' ').Append(TeamsTools.LogRef(req));
