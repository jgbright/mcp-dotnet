...
IHost BuildMcpHost(Stream? input, Stream? output)
{
    ...
    mcp
        ...
        // and wait_for_release can run for half an hour, too long to hold a request open. Every
        // other tool stays Synchronous: a task handle is a worse answer than a sub-second result.
        // Optional rather than Required means a client that never negotiated the extension still
        // gets an answer by blocking, so each waiter bounds its own wait instead of relying on
